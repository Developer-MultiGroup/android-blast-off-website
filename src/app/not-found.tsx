...
export default function NotFound() {
  ...
  return (
    <div
      className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-secondary to-primary"
    >
      ...
      >
        ...
        {clickCount > 0 && 5 - clickCount > 0 && (
          <div className="absolute -top-4 -right-4 bg-accent rounded-full w-8 h-8 flex items-center justify-center text-white text-xs font-bold">
            {5 - clickCount}
          </div>
        )}
        <Sparkles
          className="absolute -top-6 -left-6 text-accent animate-spin"
          style={{ animationDuration: "8s" }}
        />
        <Sparkles
          className="absolute -bottom-6 -right-6 text-accent animate-spin"
      ...
      {showEasterEgg && (
        <>
          ...
          >
            ...
            <p className="text-base md:text-xl text-accent mt-4 text-center max-w-md px-6">
            ...
            <Button
              ...
              className="mt-8 bg-accent hover:bg-accent/50"
            >
              Geri Dön
            </Button>
          </div>
